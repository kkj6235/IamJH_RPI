    ...
    */
    ...
    while(1)
    {
        ...
        if(strncmp(msg,"1",1)==0)
        {
            ...
            GPIOWrite(LED_2, 0);
            GPIOWrite(LED_3, 0);
            if (GPIOUnexport(LED_1) == -1) printf("LED1 unexport error\n");
            if (GPIOUnexport(LED_2) == -1) printf("LED2 unexport error\n");
            if (GPIOUnexport(LED_3) == -1) printf("LED3 unexport error\n");
            is_exit = 1;
            pthread_exit(0);
        }
    }
